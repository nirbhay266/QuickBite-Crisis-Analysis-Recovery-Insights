📊 QuickBite Crisis Analysis & Recovery Dashboard

A complete end-to-end Power BI analytics case study showing how QuickBite’s orders, revenue, customers, restaurants, delivery performance, and sentiment were impacted during the 2025 crisis.

This project analyzes the root causes, metrics, and recovery strategy through 5 interactive dashboards.

# 🏠 1. Home Navigation Dashboard
Purpose:

This is the main navigation screen of the report.
Users can click icons to move to Customer, Delivery, Restaurant, Summary, or Review dashboards.


<img width="703" height="395" alt="Home" src="https://github.com/user-attachments/assets/92a26f66-ccea-46c9-92c1-2b3edbe30d82" />


# 👥 2. Customer Impact Dashboard
Purpose:

Shows how customer behavior changed during the crisis — orders, loyalty, churn, cuisine preference, city distribution, and revenue loss.



<img width="681" height="383" alt="Customer" src="https://github.com/user-attachments/assets/d01ed836-a8cf-4618-9d06-9663676fdbfa" />




Key Customer Insights

Total Orders: 144K

Active Customers: 102K

Repeat Customers: 32K

Repeat Purchase Rate: 31.62%

Heavy churn from premium customers

City-wise activity dropped sharply in metro cities

North Indian cuisine contributed the most

# 🚚 3. Delivery Performance Dashboard
Purpose:

Analyzes delivery delays, SLA performance, cancellation patterns, and partner efficiency during the crisis.

Screenshot:

Key Delivery Insights

On-time delivery rate: 70%

Late deliveries spiked during outage

Avg delivery time slightly improved

Highest cancellation: Mumbai, Delhi

Partner ratings remained stable



<img width="682" height="385" alt="Delivery" src="https://github.com/user-attachments/assets/a4d9ce43-5a54-40e8-90db-dc20c7220be5" />


# 🍽️ 4. Restaurant Performance & Crisis Summary Dashboard
Purpose:

Evaluates restaurant-wise revenue, orders, cancellations, cuisine-level impact, and overall decline.

Screenshot:

Key Restaurant Insights

Pre-crisis orders: 110K, crisis orders: 33K

Overall decline: ~70%

Bengaluru, Chennai most impacted

Premium meals collapsed; basic meals survived

Revenue loss: ₹48.56M



<img width="680" height="384" alt="Restuarent" src="https://github.com/user-attachments/assets/bd252fae-7cc2-4228-8360-fce5233661df" />


# ⭐ 5. Review & Sentiment Impact Dashboard
Purpose:

Shows how customer reviews, ratings, and sentiment changed during the crisis.

Screenshot:

Key Review Insights

Positive Reviews: 66%

Negative Reviews: 12%

Sentiment dip exactly matched outage period

Frequent complaints:

Food quality issues

Hygiene concerns

Packaging failure

Late delivery

SLA% strongly influences ratings





<img width="681" height="383" alt="Review" src="https://github.com/user-attachments/assets/e9197802-ea53-4661-ae2d-aea2eb316338" />


# 📌 Executive Summary (Key Takeaways)

Demand & revenue dropped ~70%

Major impact: Bengaluru, Delhi, Chennai

Customers shifted from premium meals → low-value survival meals

Negative sentiment spiked during service outages

High-value loyal customers showed strongest churn




<img width="681" height="386" alt="Summary" src="https://github.com/user-attachments/assets/d97446fc-2cbb-4661-8acd-7c5dbbce32f0" />


Recovery requires:
✔ Food safety upgrades
✔ Better SLA performance
✔ Targeted incentives
✔ Strong restaurant partnerships

# 📂 Project Structure
📦 QuickBite-Crisis-Analysis
 ┣ Home.png
 ┣ Customer.png
 ┣ Delivery.png
 ┣ Summary.png
 ┣ Review.png
 ┣ README.md
 ┗ QuickBite.pbix

# 👨‍💻 Developed By
Nirbhay Kumar
